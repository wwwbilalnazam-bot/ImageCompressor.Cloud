...
/**
 * Site header — ported from PremiumHeader.jsx.
 *
 * Only the routing primitives changed: react-router-dom's `useLocation`/`Link`
 * are now next-intl's `usePathname`/`Link` (from src/i18n/navigation.js), which
 * return and emit locale-aware paths — `usePathname()` gives '/compress-pdf'
 * with the '/en' prefix already stripped, so the active-tab logic below is
 * unchanged from the original. Dark-mode and mobile-drawer behaviour are
 * carried over as-is.
 */
export default function HeaderClient() {
  ...
  const isCompressor =
    currentPath === '/' || currentPath.includes('/compress-image') || currentPath.includes('compressor')
  const isMergePdf = currentPath === '/merge-pdf'
  const isSplitPdf = currentPath === '/split-pdf'
  const isCompressPdf = currentPath === '/compress-pdf'
  const isConverter = !isCompressor && !isMergePdf && !isSplitPdf && !isCompressPdf

  const navLinks = [
    { path: '/', label: t('nav.compressor'), icon: '⚡', isActive: isCompressor },
    { path: '/converter', label: t('nav.converter'), icon: '🔄', isActive: isConverter },
    { path: '/merge-pdf', label: t('nav.mergePdf'), icon: '🔗', isActive: isMergePdf },
    { path: '/split-pdf', label: t('nav.splitPdf'), icon: '✂️', isActive: isSplitPdf },
    { path: '/compress-pdf', label: t('nav.compressPdf'), icon: '🗜️', isActive: isCompressPdf },
  ]
