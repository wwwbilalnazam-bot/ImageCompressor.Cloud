...
const PART_COLORS = ['bg-emerald-500', 'bg-blue-500', 'bg-purple-500', 'bg-amber-500', 'bg-pink-500', 'bg-cyan-500']
            ...
            {file && !results && (
              <div className="space-y-4">
                ...
                {!isSelectableMode && (
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
                    <h2 className="text-sm font-bold text-slate-900 dark:text-white">
                      Preview — pages grouped by output file
                    </h2>
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 max-h-[30rem] overflow-y-auto p-1">
                      {pages.map((page) => {
                        ...
                        return (
                          <div
                            key={page.pageNumber}
                            className="relative flex flex-col rounded-xl overflow-hidden border-2 border-slate-200 dark:border-slate-700"
                          >
                            {color && (
                              <div
                                className={`absolute top-1.5 left-1.5 z-10 px-1.5 py-0.5 rounded text-white text-[9px] font-bold ${color}`}
                              >
                                Part {partIdx + 1}
                              </div>
                            )}
                ...
                {isProcessing ? (
                  <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 text-center space-y-3">
                    <h2 className="text-sm font-bold text-slate-900 dark:text-white">{progressMsg || 'Processing...'}</h2>
                  ...
                  >
                    ...
                    <span>{outputPlan && !outputPlan.error ? `Split PDF — ${outputPlan.label}` : 'Split PDF'}</span>
                  </button>
                )}
              </div>
            )}
