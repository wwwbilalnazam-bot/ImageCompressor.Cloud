...
export default function CompressPdfPage() {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      const effectiveMaxDimension = optimizeForWeb
        ? Math.min(maxDimension || 1500, 1500)
        : maxDimension
  ...
  return (
    <div className="w-full flex-1 flex flex-col justify-between py-4 sm:py-6 px-4 container max-w-6xl mx-auto font-sans">
      <SEO
        title="Compress PDF Online Free - Reduce PDF File Size | ImageCompressor Cloud"
        description="Compress PDF files online for free. Reduce PDF size while keeping text sharp and readable — automatic, low, medium and high compression levels, with grayscale, DPI and metadata options. 100% private, processed in your browser."
        canonicalUrl="https://imagecompressor.cloud/compress-pdf"
      ...
      <div className="space-y-6">
            ...
            {!file && (
              ...
              >
                ...
                <div className="flex flex-col items-center space-y-3">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">PDF only • up to 300MB • 100% processed in your browser</p>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-3 sm:p-4 space-y-3">
                  ...
                  {showAdvanced && (
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Max image resolution</label>
                        <select
                          value={dpiId}
                          onChange={(e) => {
                ...
                {isProcessing ? (
                  <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 text-center space-y-3">
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white">{progressMsg || 'Compressing...'}</h3>
                ...
                <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-2xl p-6 shadow-lg space-y-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-bold">Compression Complete! ✨</h3>
                      <p className="text-sm text-emerald-100 mt-1">
                        {result.reduction > 0
                          ? `Reduced by ${result.reduction}% — ${result.imagesRecompressed} of ${result.totalImages} image${result.totalImages !== 1 ? 's' : ''} optimized`
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="text-center p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900">
                      ...
                      <p className="text-lg font-bold text-emerald-700 dark:text-emerald-400">{formatBytes(result.compressedSize)}</p>
                    </div>
                  </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-3 text-sm text-slate-600 dark:text-slate-300">
          ...
          <p>
            ...
            the PDF's structure rather than an image, so they're left completely untouched, keeping your document sharp
