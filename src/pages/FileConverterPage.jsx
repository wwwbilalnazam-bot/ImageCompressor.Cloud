...
const FILE_TYPE_META = {
  ...
  [PPTX_MIME]: { label: 'PowerPoint Presentation', color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-950/30' },
...
export default function FileConverterPage({ defaultTargetFormat = 'image/jpeg' }) {
  const [selectedPresetId, setSelectedPresetId] = useState('word-to-pdf')
  ...
  useEffect(() => {
    ...
    const match = CONVERSION_PRESETS.find((p) => p.to === defaultTargetFormat)
  ...
  return (
    <div className="w-full flex-1 flex flex-col justify-between py-4 sm:py-6 px-4 container max-w-5xl mx-auto font-sans">
      <SEO
        title="Free File Converter Online - PDF, Word, Excel, PowerPoint & Images"
        description="Convert PDF to Word, Excel, PowerPoint, JPG or PNG and back — with real formatting, tables and images preserved. Convert images and text files too, free and instantly."
        canonicalUrl="https://imagecompressor.cloud/converter"
        keywords="file converter, pdf to word, pdf to excel, pdf to powerpoint, word to pdf, convert pdf online"
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {!convertedFiles.length && !isProcessing && (
              ...
              >
                ...
                <div className="flex flex-col items-center space-y-3">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Supports batch conversion of multiple files</p>
                </div>
              </div>
            )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {convertedFiles.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`h-32 rounded-xl flex items-center justify-center overflow-hidden p-2 ${meta ? meta.bg : 'bg-slate-100 dark:bg-slate-950'}`}>
                          {isRenderableImage ? (
                            <img src={item.dataUrl} alt={item.filename} className="max-h-28 w-auto object-contain rounded" />
                          ) : (
                            <div className="text-center space-y-1">
                              <svg className={`w-10 h-10 mx-auto ${meta ? meta.color : 'text-slate-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                              </svg>
                              <span className={`text-[11px] font-bold block ${meta ? meta.color : 'text-slate-600 dark:text-slate-300'}`}>
          ...
          <div className="space-y-4">
            <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-2xl p-4 border border-emerald-200 dark:border-emerald-900 space-y-2">
              <h4 className="font-bold text-slate-900 dark:text-white text-sm flex items-center gap-1.5">🔒 Privacy</h4>
              <p className="text-xs text-slate-600 dark:text-slate-300">
                Your files are automatically deleted after processing. Image, PDF-preview and text conversions run
                entirely in your browser; only Office-document conversions are sent to our conversion service, and
                never stored afterward.
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
              <h4 className="font-bold text-slate-900 dark:text-white text-sm">❓ Frequently Asked Questions</h4>
